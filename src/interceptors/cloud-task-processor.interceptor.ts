...
export class CloudTaskProcessorInterceptor implements NestInterceptor {
    ...
    private getConsumerService(): ConsumerService | null {
        if (!this.consumerService) {
            try {
                this.consumerService = this.moduleRef.get(ConsumerService, { strict: false });
            } catch (error) {
                this.logger.warn('Could not resolve ConsumerService - auto-processing will be disabled');
                return null;
            }
        }
    ...
    async intercept(context: ExecutionContext, next: CallHandler) {
        ...
        if (!consumerService) {
            this.logger.warn('Cannot auto-process task: ConsumerService not available');
            return next.handle();
        }
